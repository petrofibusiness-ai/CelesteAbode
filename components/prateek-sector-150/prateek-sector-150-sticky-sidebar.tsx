...
const PROPERTY_NAME = PRATEEK_SECTOR_150_PROJECT_NAME;
const PROPERTY_SLUG = PRATEEK_SECTOR_150_SLUG;
...
export function PrateekSector150MapEmbed({ className = "" }: { className?: string }) {
  return (
    <div
    ...
    >
      <iframe
        src={MAPS_EMBED_SRC}
        title={`${PRATEEK_SECTOR_150_PROJECT_NAME} on Google Maps`}
...
export function PrateekSector150StickySidebar({
  idPrefix = "prateek",
  part = "all",
}: {
  idPrefix?: string;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const message = `WhatsApp enquiry for ${PRATEEK_SECTOR_150_PROJECT_NAME} (pre-launch). Submitted from the property page.`;
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          message,
          formSource: `property-page-${PRATEEK_SECTOR_150_SLUG}`,
            ...
            <div className="flex gap-2.5">
              ...
              <div className="min-w-0 flex-1">
                ...
                <p className="mt-1 text-[11px] leading-snug text-gray-400">
                  Submit EOI early for allotment preference. Share your details and we&apos;ll send the pre-launch brief.
                </p>
              </div>
            </div>
