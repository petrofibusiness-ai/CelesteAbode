...
const PROPERTY_NAME = PERIGON_VASUNDHARA_PROJECT_NAME;
const PROPERTY_SLUG = PERIGON_VASUNDHARA_SLUG;

const MAPS_EMBED_SRC =
  "https://www.google.com/maps?q=Sector+4,+Vasundhara,+Ghaziabad,+Uttar+Pradesh&output=embed";

export function PerigonVasundharaMapEmbed({ className = "" }: { className?: string }) {
  return (
    <div
    ...
    >
      <iframe
        src={MAPS_EMBED_SRC}
        title={`${PERIGON_VASUNDHARA_PROJECT_NAME} on Google Maps`}
...
export function PerigonVasundharaStickySidebar({
  idPrefix = "perigon",
  part = "all",
}: {
  idPrefix?: string;
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const message = `WhatsApp enquiry for ${PERIGON_VASUNDHARA_PROJECT_NAME} (pre-launch). Submitted from the property page.`;
    ...
    try {
      const res = await fetch("/api/contact", {
        ...
        body: JSON.stringify({
          ...
          message,
          formSource: `property-page-${PERIGON_VASUNDHARA_SLUG}`,
          propertyTitle: PROPERTY_NAME,
          propertyLocation: "Sector 4, Vasundhara, Ghaziabad",
        ...
        {showCallback ? (
          ...
          >
            ...
            <p className="mt-1.5 text-xs leading-relaxed text-gray-400">
              Share your name and number. We will walk you through EOI and what is still pending on RERA.
            </p>
