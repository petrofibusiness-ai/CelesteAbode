        ...
        >
          ...
          {process.env.NODE_ENV === "development" && (
            <details className="mt-6 text-left max-w-lg mx-auto">
              <summary className="cursor-pointer text-sm text-muted-foreground mb-2" style={{ fontFamily: 'Poppins, sans-serif' }}>
                Error details (development only)
              </summary>
              <pre className="mt-2 p-4 bg-muted rounded-lg text-xs overflow-auto text-left" style={{ fontFamily: 'monospace' }}>
                {error.message}
                {error.stack && `\n\n${error.stack}`}
              </pre>
            </details>
