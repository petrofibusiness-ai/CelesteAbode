...
import { MapPin, Loader2, Building2, Sparkles } from "lucide-react";
...
interface PreLaunchPropertiesGridProps {
  initialProperties: Property[];
  initialTotalCount?: number;
}

const PROPERTIES_PER_PAGE = 6;
const LOADING_EVENT = "pre-launch-properties-loading";
const FILTER_EVENT = "pre-launch-filter-change";

export function PreLaunchPropertiesGrid({
  initialProperties,
  initialTotalCount = initialProperties.length,
}: PreLaunchPropertiesGridProps) {
  ...
  const fetchPage = useCallback(async (filters: FilterState, page: number, mode: "filter" | "page") => {
    const offset = (page - 1) * PROPERTIES_PER_PAGE;
    if (mode === "filter") {
      setIsLoading(true);
      window.dispatchEvent(new CustomEvent(LOADING_EVENT, { detail: true }));
    } else {
    ...
    try {
      ...
      filters.locality.forEach((locationSlug) => {
      ...
      const response = await fetch(`/api/properties/pre-launch/search?${params.toString()}`);

      if (!response.ok) {
        throw new Error("Failed to fetch pre-launch properties");
      }
      ...
      if (mode === "filter") {
        setIsLoading(false);
        window.dispatchEvent(new CustomEvent(LOADING_EVENT, { detail: false }));
      } else {
  ...
  useEffect(() => {
    ...
    window.addEventListener(FILTER_EVENT, handleFilterChange as EventListener);
    return () => {
      window.removeEventListener(FILTER_EVENT, handleFilterChange as EventListener);
    };
  }, [fetchPage]);
    ...
    return (
      <div className="flex flex-col items-center justify-center py-16 bg-gray-50 rounded-2xl">
        ...
        <p className="text-lg text-gray-600 mb-6 font-poppins text-center">
          No pre-launch properties found matching your search criteria.
        </p>
        <Link
          href="/properties"
    ...
    <>
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-16">
          ...
          <p className="text-lg text-gray-600 font-poppins">Searching pre-launch properties...</p>
        </div>
      ) : (
                  ...
                  <Card className="border-0 bg-card overflow-hidden hover:shadow-2xl transition-all duration-500 group cursor-pointer p-0 transform hover:-translate-y-2 hover:scale-[1.02]">
                    <div className="relative w-full h-80 rounded-xl overflow-hidden">
                      ...
                      <div className="absolute top-3 left-3 z-10">
                        <span className="inline-flex items-center gap-1.5 rounded-full bg-[#CBB27A]/95 px-3 py-1 text-xs font-semibold text-black shadow-lg font-poppins">
                          <Sparkles className="w-3 h-3" />
                          Pre-Launch
                        </span>
                      </div>

                      <div className="absolute inset-0 bg-gradient-to-t from-black/90 via-black/20 to-transparent transition-opacity duration-500 group-hover:from-black/95 group-hover:via-black/30" />
